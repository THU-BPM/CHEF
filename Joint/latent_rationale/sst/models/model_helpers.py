...
from latent_rationale.sst.models.latent import(
    LatentRationaleModel,
    LatentRationaleModelForCHEF
)
...
def build_CHEFmodel(bert_type, cfg):
    emb_size = cfg["embed_size"]
    hidden_size = cfg["hidden_size"]
    dropout = cfg["dropout"]
    layer = cfg["layer"]
    ...
    lagrange_alpha = cfg["lagrange_alpha"]
    nlayer = cfg["nlayer"]
    return LatentRationaleModelForCHEF(
        bert_type=bert_type, emb_size=emb_size,
        hidden_size=hidden_size, output_size=3,
        dropout=dropout, layer=layer,
        dependent_z=dependent_z,
        selection=selection, lasso=lasso,
        lambda_init=lambda_init,
        lagrange_lr=lagrange_lr, lagrange_alpha=lagrange_alpha, nlayer=nlayer)
