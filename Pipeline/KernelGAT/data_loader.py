...
def tok2int_sent(sentence, tokenizer, max_seq_length):
    ...
    if tokens_b:
        ...
        tokens = tokens + ["[SEP]"] + tokens_b + ["[SEP]"]
        ...
        with open(data_path) as fin:
            for step, line in enumerate(fin):
                ...
                for evidence in instance['evidence']:
                    evi_list.append([
                        self.process_sent(claim), 
                        self.process_wiki_title(evidence[0]),
...
class CHEFDataLoader(DataLoader):
    def __init__(self, data_path, label_map, tokenizer, args, test=False, cuda=True, batch_size=64):
        self.evidence_type = args.evidence_type
    ...
    def read_file(self, data_path):
        ...
        for row in dataList:
            claim = row['claim']
            evs = []
            if self.evidence_type == 'gold':
                evs = [
                    ev['text'] for ev in row['gold evidence'].values()
                    if len(ev['text']) > 0
                ]
            elif self.evidence_type == 'tfidf':
                evs = row['tfidf']
            elif self.evidence_type == 'cossim':
                evs = row['sim']
            elif self.evidence_type == 'ranksvm':
                evs = row['rank_svm']
            elif self.evidence_type == 'snippet':
                evs = row['snippets']
            if len(evs) == 0:
                continue
            ...
            examples.append([evi_list, row['label']])
        return examples
