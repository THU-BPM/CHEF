...
from transformers import AutoModel, AutoModelForMaskedLM, AutoTokenizer, BertTokenizer
...
from data_loader import DataLoader, CHEFDataLoader
...
def eval_model(model, validset_reader):
    model.eval()
    correct_pred = 0.0
    all_ground_truth = torch.tensor([]).int()
    all_prediction = torch.tensor([]).int()
    for index, data in enumerate(validset_reader):
        inputs, lab_tensor = data
        prob = model(inputs)
        out_classes = prob.max(dim=1)[1]
        all_ground_truth = torch.cat([all_ground_truth, lab_tensor.detach().cpu().int()])
        all_prediction = torch.cat([all_prediction, out_classes.detach().cpu().int()])
...
def train_model(model, ori_model, args, trainset_reader, validset_reader):
    ...
    for epoch in range(int(args.num_train_epochs)):
        ...
        for index, data in enumerate(trainset_reader):
            ...
            #     with torch.no_grad():
            #         dev_accuracy, dev_f1 = eval_model(model, validset_reader)
        ...
        with torch.no_grad():
            ...
            if dev_f1 > best_f1:
                ...
                torch.save({'epoch': epoch,
                            ...
                            'best_f1': best_f1}, 
                            save_path + ".best.pt")
                logger.info("Saved best epoch {0}, best f1 {1}".format(epoch, best_f1))
...
if __name__ == "__main__":
    ...
    trainset_reader = CHEFDataLoader(
        args.train_path, label_map, tokenizer, 
        args, batch_size=args.train_batch_size)
    logger.info("loading validation set")
    # validset_reader = DataLoader(
    validset_reader = CHEFDataLoader(
        args.valid_path, label_map, tokenizer, 
        args, batch_size=args.valid_batch_size, test=True)
